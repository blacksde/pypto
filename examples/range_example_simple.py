#!/usr/bin/env python3
"""
PyPTO pl.range() 完整示例：从 Python 代码到 IR 生成（简化版）

这个例子展示了如何使用 pl.range() 创建循环并查看生成的 IR

运行方法：
1. 首先构建 PyPTO：cmake --build build --parallel
2. 运行此脚本：python examples/range_example_simple.py
"""
...
def example_simple_range():
    ...
    if hasattr(simple_loop, 'return_type'):
    ...
    if for_stmt:
        print(f"\n  ForStmt 分析:")
        print(f"    循环变量: {for_stmt.loop_var}")
